...
def parse_args_and_config():
    ...
    parser.add_argument(
        "--deg", type=str, default='denoising', required=False, help="Degradation: sr_averagepooling, denoising"
    )
    parser.add_argument("--deg_scale", type=float, default=2, help="deg_scale")    
    parser.add_argument(
        "-i",
        "--input_folder",
        type=str,
    ...
    parser.add_argument('--model_path', type=str, default='ckpt/CT.pt', help='Path to the model')
    ...
    args.output_folder = join(args.output_folder, 'CT')
...
for img_name in tqdm.tqdm(img_names):
    ...
    with torch.no_grad():
        ...
        x, _ = ddnm_plus_diffusion(x, model, betas, args.eta, A_funcs, y, sigma_y, cls_fn=None, classes=None, config=config)
