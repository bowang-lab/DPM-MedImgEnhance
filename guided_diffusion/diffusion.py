...
from functions import get_dataset, data_transform, inverse_data_transform
...
from guided_diffusion.script_util import create_model, create_classifier, classifier_defaults, args_to_dict, model_and_diffusion_defaults
import random
...
class Diffusion(object):
    ...
    def sample(self, simplified):
        ...
        if self.config.model.type == 'simple':
            ...
            if self.config.data.dataset == "CIFAR10":
                ...
                name = 'celeba_hq'
            elif self.config.data.dataset == 'LowDoseCT':
                name = 'LowDoseCT'
            else:
                raise ValueError
            if name != 'celeba_hq':
            ...
            if self.config.model.class_cond:
                ...
                ckpt = os.path.join(self.args.exp, "logs/lowdoseCT/ema_0.9999_620000.pt")
    ...
    def simplified_ddnm_plus(self, model, cls_fn):
        ...
        # if config.data.dataset == 'LowDoseCT':
        #     dataset = torchvision.datasets.ImageFolder(os.path.join(args.exp, 'datasets', 'LowDoseCT'),
        #     transform=transforms.Compose([transforms.ToTensor()]))
        #     test_dataset = dataset
        # else:
        #     dataset, test_dataset = get_dataset(args, config)
        img_names = sorted(os.listdir(args.path_y))
        ...
        if args.subset_start >= 0 and args.subset_end > 0:
            ...
            args.subset_start = 0
            args.subset_end = len(img_names)

        print(f'Dataset has size {len(img_names)}')
        ...
        avg_psnr = 0.0
        pbar = tqdm.tqdm(img_names)
        for img_name in tqdm.tqdm(img_names):
            img_np = io.imread(join(args.path_y, img_name))
            img_np = img_np/np.max(img_np)
            x_orig = torch.from_numpy(img_np).float().permute(2,0,1).unsqueeze(0).to(self.device)
            ...
            for i in range(len(Apy)):
                tvu.save_image(
                    ...
                    os.path.join(self.args.image_folder, 'Apy_'+img_name)
                )
                tvu.save_image(
                    inverse_data_transform(config, x_orig[i]),
                    os.path.join(self.args.image_folder, "orig_"+img_name)
                )
            ...
            tvu.save_image(
                x[0], os.path.join(self.args.image_folder, img_name)
            )
        ...
        for img_name in tqdm.tqdm(img_names):
            ...
            with torch.no_grad():
                if sigma_y==0.: # noise-free case, turn to ddnm
                    x, _ = ddnm_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, cls_fn=cls_fn, classes=None, config=config)
                else: # noisy case, turn to ddnm+
                    x, _ = ddnm_plus_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, sigma_y, cls_fn=cls_fn, classes=None, config=config)
            ...
            for j in range(x[0].size(0)):
                tvu.save_image(
                    x[0][j], os.path.join(self.args.image_folder, img_name)
                )
